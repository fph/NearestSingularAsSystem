function [AplusDelta, Delta] = nearest_singular_sparse_newtonlike_minres_unit_v(A, P, uv0)
%
% Computes the nearest singular matrix to A with a specified sparsity
% pattern.
% The norm of v is fixed and equal to 1.
%
...
for k = 1:10
    ...
    %mat = [diag(k1) A+2*Delta; (A+2*Delta)' diag(k2)+beta*(v*v'*2+normv2m1*eye(length(k2)))];
    % [L, U] = ilu(mat);
    matop = @(uv) [
        k1.*uv(1:n) + (A+2*Delta)*uv(n+1:2*n);
        (A+2*Delta)'*uv(1:n) + k2.*uv(n+1:2*n) + beta*(v*(v'*uv(n+1:2*n))*2 + normv2m1*uv(n+1:2*n));
        ];
